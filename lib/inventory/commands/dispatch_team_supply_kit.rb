...
module Inventory
  module Commands
    class DispatchTeamSupplyKit < ApplicationCommand
      Result = Data.define(:dispatch, :message)

      def initialize(campaign:, care_team:, dispatch_date:)
        @campaign = campaign
        @care_team = care_team
      ...
      def call
        ...
        write_transaction do
          ...
          emit_supplies_dispatched!(dispatch.id, routes.size)
        end
      ...
      def emit_supplies_dispatched!(dispatch_id, route_count)
        RecordPlatformEvent.call(
          event_type: Cidadaobr::KafkaTopics::VISIT_ROUTE_SUPPLIES_DISPATCHED,
          aggregate_type: "TeamSupplyDispatch",
          aggregate_id: dispatch_id,
          payload: {
            campaign_id: @campaign.id,
            care_team_id: @care_team.id,
            dispatch_date: @dispatch_date.iso8601,
