...
RSpec.describe Reference::Gate do
  it "runs import chain and publishes a valid release" do
    release = described_class.run!(sigtap_competence: "202605")
    ...
    expect(release.sigtap_competence).to eq("202605")
    expect(release.checksum).to be_present
  end

  it "is idempotent on second run" do
    first = described_class.run!(sigtap_competence: "202605")
    second = described_class.run!(sigtap_competence: "202605")

    expect(second.id).to eq(first.id)
  end

  it "publish_release! skips domain and sigtap import" do
    described_class.run!(sigtap_competence: "202605")
    domain_imports = 0
    sigtap_imports = 0
    ...
    release = described_class.publish_release!(sigtap_competence: "202605")
